...
class coco(IMDB):
    ...
    def _get_ann_file(self):
        ...
        #prefix = 'instances' if 'unlabeled' not in self.image_set else 'image_info'
    ...
    def gt_roidb(self):
        ...
        cache_file = os.path.join(self.cache_path, self.data_name + '_gt_roidb.pkl')
        if os.path.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                roidb = cPickle.load(fid)
            print '{} gt roidb loaded from {}'.format(self.data_name, cache_file)
            return roidb
    ...
    def evaluate_detections(self, detections, ann_type='bbox', all_masks=None):
        ...
        res_file = os.path.join(res_folder, 'detections_%s_results_12epoch_semi_0.5_co_0.1_soft.json' % self.image_set)
